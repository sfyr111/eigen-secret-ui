...
import {__DEFAULT_ALIAS__, rawMessage, SESSION_DURATION, signEOASignature} from '@eigen-secret/core/dist-browser/utils';
...
class SecretManager {
    ...
    async initSDK({alias, password = '<your password>', user, isCreate = false}) {
        ...
        const {
            signature,
            signatureTimestamp
        } = await this.getSignature(user, address, timestamp, alias == __DEFAULT_ALIAS__);
    ...
    async deposit({alias, assetId, password = '<your password>', value, user, receiver, decimals = 18}) {
    ...
    async withdraw({alias, assetId, password = '<your password>', value, user, receiver, decimals = 18}) {
